...
```javascript 
let a = ' O código avalia a expressão booleana, imprime o resultado `false`, calcula a soma dos números de 1 a 5 e imprime o resultado no console.'
print(a)
...
```javascript
let a = 'As funções calcularOrcamento() e calcularOrcamento2() teriam a mesma saída: 'Seu saldo é negativo de -1050.''
print(a)
...
```javascript
let c = 'O código verifica se o número é par, ímpar ou divisível por 3. Se for par, exibe a mensagem "O número é par!". Se for divisível por 3, exibe a mensagem "O número é divisível por 3!". Se for ímpar, exibe a mensagem "O número é ímpar e não é divisível por 3!"'
print(c)
...
```javascript
let b = 'Compra 1 aprovada. Saldo restante: 800
...
Escolha a opção que responde corretamente:
```javascript
let b = 'Simular interações físicas realistas, como colisões e movimentos, em jogos 2D.
'
print(b)
...
variavel idade = numero

se (idade < 16) {
    console.log('não pode votar');
}
se não (idade >= 16 && idade < 18) {
    console.log(voto não obrigatorio);
}
se não (console.log(voto obrigatorio));
______
...
Classe Retangulo herda de FormaGeometrica:
    ...
    Método Construtor(cor, altura, largura):
        Chamar o metodo da classe FormaGeometrica 'cor'
        definir valor altura e largura
...
Classe Circulo herda de FormaGeometrica:
    ...
    Método Construtor(cor, raio):
        Chama o metodo da classe FormaGeometrica çor
        definir valor do raio
